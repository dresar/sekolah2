...
<script type="text/javascript">
      ...
      fields: [
         {
            ...
            renderer:function(row) {
               if (row.variable == 'sendgrid_username') {
                  return A(_form1 + '.OnEdit(' + row.id + ')');
               }
               if (row.variable == 'sendgrid_password') {
                  return A(_form2 + '.OnEdit(' + row.id + ')');
               }
               if (row.variable == 'sendgrid_api_key') {
         ...
         { header:'Sendgrid Setting', renderer: 'description' },
         { header:'Value', renderer: 'value' },
      ],
      can_add: false,
      can_delete: false,
      can_restore: false,
   ...
   */
   new FormBuilder( _form1 , {
      controller:'settings/mail_server',
      fields: [
         { label:'Sendgrid User Name', name:'value' }
      ]
   });
   ...
   */
   new FormBuilder( _form2 , {
      controller:'settings/mail_server',
      fields: [
         { label:'Sendgrid Password', name:'value' }
      ]
   });
   ...
   */
   new FormBuilder( _form3 , {
      controller:'settings/mail_server',
      fields: [
         { label:'Sendgrid API Key', name:'value' }
      ]
   });
</script>
