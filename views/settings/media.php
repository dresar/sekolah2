...
<script type="text/javascript">
    var _grid = 'OPTIONS',
        ...
        _form25 = _grid + '_FORM25', // image Headmaster
        _form26 = _grid + '_FORM26', // image Headmaster
        _form27 = _grid + '_FORM27', // image Testimoni
        _form28 = _grid + '_FORM28', // image Testimoni
        ...
        fields: [
            {
                ...
                renderer:function(row) {
                    ...
                    if (row.variable == 'image_headmaster_height') {
                        return A(_form25 + '.OnEdit(' + row.id + ')');
                    }
                    if (row.variable == 'image_headmaster_width') {
                        return A(_form26 + '.OnEdit(' + row.id + ')');
                    }
                    if (row.variable == 'image_testimoni_height') {
                        return A(_form27 + '.OnEdit(' + row.id + ')');
                    }
                    if (row.variable == 'image_testimoni_width') {
    ...
    new FormBuilder( _form25 , {
        controller:'settings/media',
        fields: [
            { label:'Tinggi Gambar Headmaster (px)', name:'value', type:'number' }
        ]
    });
    ...
    new FormBuilder( _form26 , {
        controller:'settings/media',
        fields: [
            { label:'Lebar Gambar Headmaster (px)', name:'value', type:'number' }
        ]
    });
    ...
    new FormBuilder( _form27 , {
        controller:'settings/media',
        fields: [
            { label:'Tinggi Gambar Testimoni (px)', name:'value', type:'number' }
        ]
    });
    ...
    new FormBuilder( _form28 , {
        controller:'settings/media',
        fields: [
            { label:'Lebar Gambar Testimoni (px)', name:'value', type:'number' }
        ]
    });
