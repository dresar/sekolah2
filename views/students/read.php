...
<script type="text/javascript">
	...
	new GridBuilder( _grid , {
	  ...
	  resize_column: 6,
	  to_excel: true,
	  extra_buttons: '<button title="Create All Student Account" onclick="create_student_accounts()" class="btn btn-success btn-sm" data-toggle="tooltip" data-placement="top"><i class="fa fa-key"></i></button>'
	});
	...
	function create_student_accounts() {
	 	eModal.confirm('Nama Pengguna dan Kata Sandi ' + (parseInt(_SCHOOL_LEVEL) == 5 ? 'Mahasiswa' : 'Peserta Didik') + ' akan digenerate dengan menggunakan ' + (parseInt(_SCHOOL_LEVEL) == 5 ? 'NIM' : 'NIS') + '. Apakah anda yakin akan mengaktifkan seluruh akun Peserta Didik ?', 'Konfirmasi').then(function() {
	 		$('body').addClass('loading');
			$.post(_BASE_URL + 'students/students/create_student_accounts', {}, function( response ) {
