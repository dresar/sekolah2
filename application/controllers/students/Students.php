...
class Students extends Admin_Controller {
	...
	/**
	  ...
	  * Insert students record to users
	  */
	public function create_student_accounts() {
		$field = 'nis';
		if (NULL !== $this->session->userdata('school_level') && $this->session->userdata('school_level') == 5) {
			$field = 'nim';
		}
		$response = [];
		$success = 0;
		...
		$fill_data = [];
		foreach($query->result() as $row) {
			$user_name_exist = $this->model->isValExist('user_name', $row->$field, 'users');
			$email_exist = $this->model->isValExist('user_email', $row->email, 'users');
			if (!$user_name_exist && !$email_exist) {
				$fill_data[] = [
					'user_name' => $row->$field,
					'user_password' => password_hash($row->$field, PASSWORD_BCRYPT),
