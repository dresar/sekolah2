...
class Testimoni extends Admin_Controller {
	...
	public function __construct() {
		...
		$this->pk = M_testimoni::$pk;
		$this->table = M_testimoni::$table;
	}
	...
	public function index() {
		$this->vars['title'] = 'TESTIMONI';
		$this->vars['blog'] = $this->vars['testimoni'] = true;
		$this->vars['content'] = 'testimoni/read';
	...
	public function pagination() {
		...
		$query = $this->m_testimoni->get_where($keyword, $limit, $offset);
		$sql = $this->db->last_query();
		$total_rows = $this->m_testimoni->total_rows($keyword);
	...
	private function validation() {
		...
		$val->set_rules('status', 'Status', 'trim');
	...
	public function upload() {
		...
		if ($id && $id > 0 && ctype_digit((string) $id)) {
			...
			$config = [];
			$config['upload_path'] = './media_library/testimoni/';
			...
			if ( ! $this->upload->do_upload('file')) {
				...
				if ($update) {
					// chmood new file
					@chmod(FCPATH.'media_library/testimoni/'.$file['file_name'], 0777);
					// chmood old file
					@chmod(FCPATH.'media_library/testimoni/'.$file_name, 0777);
					// unlink old file
					@unlink(FCPATH.'media_library/testimoni/'.$file_name);
					// resize new image
					$this->image_resize(FCPATH.'media_library/testimoni', $file['file_name']);
				}
	...
	/**
	 ...
	 private function image_resize($source, $file_name) {
		...
		$config['width'] = intval($this->session->userdata('image_testimoni_width'));
		$config['height'] = intval($this->session->userdata('image_testimoni_height'));
